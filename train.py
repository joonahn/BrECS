...
def train_model(
	...
	for step, (data, epoch) in enumerate(scheduler, resume_step):  # step starts from load_step
		...
		if scheduler.use_buffer:
			# GCA training
			if config.get('lego_loss') == "bce_supervised":
				train_loss, data_next = model.learn_sup(data, step)
			else:
		...
		# model learns
		print('\r[Epoch {:4}, Step {:7}, Loss {:5}, t_train: {:4.1f}]'.format(
			epoch, step + 1, '%.4f' % train_loss, t_train), end='', flush=True
		)
