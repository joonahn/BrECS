...
class Res16UNetBase(MinkowskiNetwork):
	...
	def __init__(self, config):
		...
		elif model_name in ('gca', 'lego_transition'):
...
class Res16UNetNoSkipBase(Res16UNetBase):
	def network_initialization(self, in_channels, out_channels, D):
		# Setup net_metadata
		dilations = self.DILATIONS
		bn_momentum = 0.02
...
class Res16UNetNoSkipNoScaleBase(Res16UNetNoSkipBase):
	def network_initialization(self, in_channels, out_channels, D):
		# Setup net_metadata
		dilations = self.DILATIONS
		bn_momentum = 0.02
		...
		self.conv1p1s2 = ME.MinkowskiConvolution(
			self.inplanes, self.inplanes,
			kernel_size=self.kernels[1], dimension=D
		)
		...
		self.conv2p2s2 = ME.MinkowskiConvolution(
			self.inplanes, self.inplanes,
			kernel_size=self.kernels[2], dimension=D
		)
		self.bn2 = ME.MinkowskiBatchNorm(self.inplanes)
		...
		self.conv3p4s2 = ME.MinkowskiConvolution(
			self.inplanes, self.inplanes,
			kernel_size=self.kernels[3], dimension=D
		)
		...
		self.conv4p8s2 = ME.MinkowskiConvolution(
			self.inplanes, self.inplanes,
			kernel_size=self.kernels[4], dimension=D
		)
		self.bn4 = ME.MinkowskiBatchNorm(self.inplanes)
		self.block4 = self._make_layer(
			self.BLOCK, self.PLANES[3], self.LAYERS[3]
		...
		self.convtr4p16s2 = ME.MinkowskiConvolutionTranspose(
			self.inplanes, self.PLANES[4],
			kernel_size=self.kernels[5], dimension=D
		)
		self.bntr4 = ME.MinkowskiBatchNorm(self.PLANES[4])
		...
		self.convtr5p8s2 = ME.MinkowskiConvolutionTranspose(
			self.inplanes, self.PLANES[5],
			kernel_size=self.kernels[6], dimension=D
		)
		self.bntr5 = ME.MinkowskiBatchNorm(self.PLANES[5])
		...
		self.convtr6p4s2 = ME.MinkowskiConvolutionTranspose(
			self.inplanes, self.PLANES[6],
			kernel_size=self.kernels[7], dimension=D
		)
		self.bntr6 = ME.MinkowskiBatchNorm(self.PLANES[6])
		...
		self.convtr7p2s2 = ME.MinkowskiConvolutionTranspose(
			self.inplanes, self.PLANES[7],
			kernel_size=self.kernels[8], dimension=D
		)
		self.bntr7 = ME.MinkowskiBatchNorm(self.PLANES[7])
...
class Res16UNet34NoSkip(Res16UNetNoSkipBase):
	BLOCK = BasicBlock
	LAYERS = (2, 3, 4, 6, 2, 2, 2, 2)

class Res16UNet34NoSkipNoScale(Res16UNetNoSkipNoScaleBase):
	BLOCK = BasicBlock
	LAYERS = (2, 3, 4, 6, 2, 2, 2, 2)
...
class Res16UNet34NoSkipC(Res16UNet34NoSkip):
	name = 'Mink16UNet34NoSkipC'
	PLANES = (32, 64, 128, 256, 256, 128, 96, 96)

class Res16UNet34NoSkipNoScaleC(Res16UNet34NoSkipNoScale):
	name = 'Mink16UNet34NoSkipNoScaleC'
	PLANES = (32, 64, 128, 256, 256, 128, 96, 96)
