...
import brick
import rules
from lego import utils

# import rules_v3
import rules

def convert_to_bricks(X, A, max_bricks=None, do_validate=True):
    list_bricks = []
...
class Bricks(object):
    ...
    def sample_one(self):
        ...
        cur_rule = np.random.choice(rules_v1.RULE_CONTACTS_2_4, p=rules_v1.PROBS_CONTACTS_2_4)
...
#        for trans in translations:
    ...
    def compute_adjacency_degree_matrices(self):
        ...
        for ind_1, brick_1 in enumerate(bricks_):
            ...
            for ind_2, brick_2 in enumerate(bricks_):
                ...
                # if np.abs(pos_1[2] - pos_2[2]) == 1 and np.linalg.norm(pos_1 - pos_2) < 4:
                if np.abs(pos_1[2] - pos_2[2]) == 1:
                    vert_1 = brick_1.get_vertices()
                    vert_2 = brick_2.get_vertices()
                    min_max_1 = utils.get_min_max_3d(vert_1)
                    ...
                    if utils.check_overlap_2d(min_max_1[:2], min_max_2[:2]) == 1:
                        A[ind_1, ind_2] = 1
    ...
    def compute_adjacency_matrices_from_prev_graph(self, prev_A):
        bricks_ = self.get_bricks()
        len_bricks = self.get_length()

        A = np.zeros((len_bricks, len_bricks))
        A[:len_bricks - 1, :len_bricks - 1] = prev_A

        ind_last = len(bricks_) - 1
        brick_last = bricks_[ind_last]

        for ind_1, brick_1, ind_2, brick_2 in [(ind_last, brick_last, ind, brk) for ind, brk in enumerate(bricks_)] + [(ind, brk, ind_last, brick_last) for ind, brk in enumerate(bricks_)]:
            if ind_1 == ind_2:
                continue
    ...
    def get_graph_from_prev_graph(self, X, A):
        """
        intended to increase performance
        """
        assert X.shape[0] == (self.get_length() - 1)
        assert A.shape[0] == (self.get_length() - 1)
        X = self.get_positions()
        A = self.compute_adjacency_matrices_from_prev_graph(A)

        return X, A


    def sample_one_from_pivot(self, pivot):
        bricks = self.get_bricks()
        brick_sampled = np.random.choice(bricks)
        # brick_sampled = bricks[-1]
